...
class ZoomableSvg extends Component {
  state = {
    zoom: 1,
    left: 0, // vertice superior esquerdo
    top: 0, // vertice superior esquerdo
  }
      ...
      onPanResponderMove: evt => {
        ...
        if (length === 1) { // Apenas moveu
          const [{ locationX, locationY }] = touches;
            const coordenadas = {
                top: Math.round(locationY),
                left: Math.round(locationX)
            }
  ...
  render() {
    ...
    return (
        <View>
            <View {...this._panResponder.panHandlers} style={{backgroundColor: 'red'}}>
                <Svg
                width={width}
                height={height}
                viewBox="0 0 65 65"
